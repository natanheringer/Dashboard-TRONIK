...
from banco_dados.modelos import Coletor, Coleta, Sensor, Parceiro
...
class TestPerformanceQueries:
    ...
    def test_query_lixeiras_com_join_performance(self, db_session):
        ...
        if coletores:
            ...
            inicio2 = time.time()
            for l in coletores[:10]:  # Testar apenas 10 primeiras
                _ = l.parceiro
                _ = l.tipo_material
...
def test_paginacao_variada_performance(db_session, pagina, por_pagina):
    ...
    assert len(coletores) <= por_pagina, f"Página retornou mais itens que o esperado"
