...
def validar_range(valor: float, min_val: float | None = None, max_val: float | None = None, nome_campo: str = "campo") -> None:
    """
...
def validar_tamanho_string(valor: str, min_len: int | None = None, max_len: int | None = None, nome_campo: str = "campo") -> None:
    """
...
def validar_id_positivo(valor: Any, nome_campo: str = "id") -> int:
    ...
    try:
        ...
        raise ErroValidacao(f"Campo '{nome_campo}' deve ser um número inteiro") from err
...
def validar_data_iso(data_str: str, nome_campo: str = "data") -> datetime:
    ...
    try:
        ...
        raise ErroValidacao(
            f"Campo '{nome_campo}' deve estar no formato ISO 8601 (ex: 2025-11-24T10:00:00)"
        ) from err
...
def validar_paginacao(pagina: int | None = None, por_pagina: int | None = None) -> tuple[int, int]:
    """
    ...
    try:
        ...
        raise ErroValidacao("Parâmetros de paginação devem ser números inteiros") from err
...
def validar_dados_requisicao(dados: dict | None, campos_obrigatorios: list[str] = None) -> dict:
    """
